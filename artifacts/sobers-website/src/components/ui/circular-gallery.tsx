...
class App {
  ...
  renderer!: Renderer; gl!: OGLRenderingContext; camera!: Camera;
  scene!: Transform; planeGeometry!: Plane; mediasImages!: GalleryItem[];
  ...
  createRenderer() {
    ...
    if (!this.renderer || !this.renderer.gl) throw new Error("WebGL context unavailable");
  ...
  useEffect(() => {
    ...
    try {
      ...
      app = new App(containerRef.current, {
        items, bend, textColor: computedColor, borderRadius,
        font: computedFont, scrollSpeed, scrollEase,
      });
    } catch {
      // WebGL unavailable in this environment — gallery silently absent
      return;
    }
    return () => { app?.destroy(); };
